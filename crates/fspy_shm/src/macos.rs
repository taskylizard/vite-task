...
/// Creates a POSIX shared-memory mapping of `size` bytes.
///
/// # Errors
///
...
fn create_with(size: usize, mut next_id: impl FnMut() -> String) -> io::Result<CreatedShm> {
    ...
    loop {
        ...
        return Ok(CreatedShm { shm: Shm { name: ShmName::Owner(owner), mapping } });
    }
}
...
mod tests {
    ...
    fn non_page_aligned_size_reopens_exactly() {
        let owner = create(100).unwrap().shm;
    ...
    fn name_collisions_are_retried() {
        let existing = create(SIZE).unwrap().shm;
        let fresh_id = loop {
            let id = new_id(SIZE);
            if id != existing.id() {
                break id;
        ...
        assert_eq!(created.shm.id(), fresh_id);
    }
